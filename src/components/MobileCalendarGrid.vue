...
const visibleMonth = ref((props.value || props.max).slice(0, 7))

watch(() => props.value, value => {
  if (value) visibleMonth.value = value.slice(0, 7)
})
...
const canMoveNextMonth = computed(() => visibleMonth.value < props.max.slice(0, 7))
...
<style scoped>
.mobile-calendar-grid { padding: 0 12px 12px; color: var(--ios-label); }
