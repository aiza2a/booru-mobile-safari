<template>
  <div v-if="visible" class="mobile-date-filter">
    <v-btn-toggle v-if="showModeToggle" v-model="dateFilter.mode" mandatory dense class="date-mode-toggle">
      <v-btn :value="primaryMode" small>{{ primaryLabel }}</v-btn>
      <v-btn value="date" small>{{ secondaryLabel }}</v-btn>
    </v-btn-toggle>
    <v-menu offset-y>
    ...
    <v-btn v-if="dateFilter.mode === 'date'" icon small aria-label="应用日期筛选" @click="apply"><v-icon>{{ mdiCheck }}</v-icon></v-btn>
  </div>
</template>
...
const isHomeLike = computed(() => routeKind === 'home' || routeKind === 'random')
const showModeToggle = computed(() => hasLatestMode.value || isHomeLike.value)
const primaryMode = computed<DateFilterMode>(() => hasLatestMode.value ? 'latest' : 'all')
const primaryLabel = computed(() => hasLatestMode.value ? (routeKind === 'ranked' ? '近期' : '最近') : '全部')
const secondaryLabel = computed(() => routeKind === 'ranked' ? '历史高分' : '日期')
...
watch(() => dateFilter.mode, mode => {
  if (mode === primaryMode.value) apply()
})
</script>
